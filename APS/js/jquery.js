...
var isFunction = função isFunction (obj) {
      ...
      // (isto é, `tipo de documento.criarElement (" objeto ") ===" função "`).
      // Não queremos classificar * any * DOM como uma função.
      retornar typeof obj === "function" && typeof obj.nodeType! == "number";
  };
...
var documento = window.document;
	...
	função DOMEval (código, nó, doc) {
		...
		script.text = código;
		if (nó) {
			for (i em preservedScriptAttributes) {

				// Suporte: Firefox 64+, Edge 18+
				// Alguns navegadores não suportam a propriedade "nonce" nos scripts.
				...
				// se torna conectado ao contexto de navegação.
				// Consulte https://github.com/whatwg/html/issues/2369
				// Veja https://html.spec.whatwg.org/#nonce-attributes
				// A verificação `node.getAttribute` foi adicionada por uma questão de
				// `jQuery.globalEval` para que ele possa fingir um nó que não contém
				// via um objeto.
...
// sem proteção em outro lugar, parece mais seguro definir global somente para este módulo
...
var
	...
	jQuery = função (seletor, contexto) {
		...
		// Precisa de init se o jQuery for chamado (apenas permita que o erro seja gerado, se não incluído)
...
jQuery.fn = jQuery.prototype = {
	...
	get: function (num) {
		...
		retornar num <0? this [num + this.length]: este [num];
	}
	...
	pushStack: function (elems) {
		...
		var ret = jQuery.merge (este.construtor (), elems);
	...
	each: function (retorno de chamada) {
	...
	função par() {
		retorna this.pushStack (jQuery.grep (this, function (_elem, i) {
			retorno (i + 1)% 2;
		}));
	}
	...
	eq: function (i) {
		...
		retorne this.prevObject || this.constructor ();
	}
...
jQuery.extend = jQuery.fn.extend = function () {
	...
	if (typeof target === "boolean") {
		...
		// Ignora o booleano e o destino
		alvo = argumentos [i] || {};
		i ++;
	}

	// Manipula maiúsculas e minúsculas quando o destino é uma string ou algo assim (possível em cópia detalhada)
	...
	para (; i <comprimento; i ++) {
		...
		if ((opções = argumentos [i])! = nulo) {
			...
			para (nome nas opções) {
				...
				// Prevenir poluição de Object.prototype
				...
				if (profundidade && cópia && (jQuery.isPlainObject (cópia) ||
...
jQuery.extend ({
	...
	isPlainObject: function (obj) {
		...
		// Objetos com protótipo são simples se eles foram construídos por uma função global Object
	...
	// Avalia um script em um contexto fornecido; volta ao global
	...
	grep: function (elems, retorno de chamada, inverter) {
		var callbackInverse,
			corresponde = [],
			i = 0
			length = elems.length,
	...
	map: function (elems, callback, arg) {
		...
		// Percorre a matriz, traduzindo cada um dos itens para seus novos valores
...
jQuery.each ("Número booleano String Função Matriz Data RegExp Object Error Symbol" .split (""),
função (_i, nome) {
...
var i,
	...
	// Use um índice simplificado, pois é mais rápido que o nativo
	...
	booleans = "marcado | selecionado | assíncrono | foco automático | reprodução automática | controles | adiar | desativado | desativado | oculto |" +
		"ismap | loop | múltiplo | aberto | somente leitura | necessário | escopo"
	...
	pseudos = ":(" + identificador + ") (?: \\ ((" +
		...
		// Para reduzir o número de seletores que precisam de tokenizar no pré-filtro, prefira argumentos:
	...
	// Espaço em branco à esquerda e à esquerda, capturando alguns caracteres que não sejam espaços em branco que precedem o último
	rwhitespace = new RegExp (espaço em branco + "+", "g"),
	...
	matchExpr = {
		...
		"CHILD": new RegExp ("^ :( somente | primeiro | último | enésimo | enésimo-último) - (filho | do tipo) (?: \\ (" +
	...
	rhtml = / HTML $ / i,
	entradas de entrada = / ^ (?: input | select | textarea | button) $ / i,
	cabeçalho = / ^ h \ d $ / i,
	...
	funescape = função (escape, não Hex) {
	...
	fcssescape = function (ch, asCodePoint) {
		if (asCodePoint) {
			...
			// U + 0000 NULL torna-se U + FFFD CHARACTER DE SUBSTITUIÇÃO
			if (ch === "\ 0") {
				retornar "\ uFFFD";
			}

			// Caracteres de controle e números (dependendo da posição) são escapados como pontos de código
			retornar ch.slice (0, -1) + "\\" +
				ch.charCodeAt (ch.length - 1) .toString (16) + "";
		}

		// Outros caracteres ASCII potencialmente especiais são escapados pela barra invertida
		retornar "\\" + ch;
	}

	// Usado para iframes
	// Veja setDocument ()
	// A remoção do wrapper de função causa uma "Permissão Negada"
	// erro no IE
	...
	inDisabledFieldset = addCombinator (
		função (elem) {
			retornar elem.disabled === true && elem.nodeName.toLowerCase () === "fieldset";
		}
...
tentar {
	push.apply (
		(arr = slice.call (preferidoDoc.childNodes)),
		selectedDoc.childNodes
	);

	// Suporte: Android <4.0
	// Detecta push.apply com falha silenciosa
	// eslint-disable-next-line sem expressões não utilizadas
	arr [selectedDoc.childNodes.length] .nodeType;
} captura (e) {
	push = {apply: arr.length?
		...
		// Suporte: IE <9
		// caso contrário, adicione diretamente
		função (alvo, els) {
			var j = target.length,
				i = 0;
...
função Sizzle (seletor, contexto, resultados, semente) {
	var m, i, elem, nid, partida, grupos, newSelector,
		newContext = context && context.ownerDocument,

		// nodeType assume como padrão 9, pois o contexto assume como padrão o documento
		nodeType = contexto? context.nodeType: 9;

	resultados = resultados || [];

	// Retorna antecipadamente de chamadas com seletor ou contexto inválido
	if (tipo de seletor! == "string" ||! seletor ||
		nodeType! == 1 && nodeType! == 9 && nodeType! == 11) {

		retornar resultados;
	}

	// Tente atalho encontrar operações (ao contrário de filtros) em documentos HTML
	if (! semente) {
		...
		if (documentIsHTML) {
			...
			// Se o seletor for suficientemente simples, tente usar o método DOM "get * By *"
			...
			if (support.qsa &&
				...
				if (nodeType === 1 &&
					...
					// Prefixa todos os seletores na lista
	...
	// Todos os outros
	retornar select (selector.replace (rtrim, "$ 1"), contexto, resultados, semente);
}

/ **
 * Crie caches de valor-chave de tamanho limitado
 * @returns {function (string, object)} Retorna os dados do Object após armazená-los em si mesmos com
 * nome da propriedade a sequência (com sufixo de espaço) e (se o cache for maior que Expr.cacheLength)
...
função createCache () {
	...
	cache de função (chave, valor) {
		...
		// Use (tecla + "") para evitar colisão com propriedades de protótipo nativas (consulte a edição # 157)
...
/ **
 * Marque uma função para uso especial da Sizzle
...
/ **
 * Suporte ao teste usando um elemento
 * @param {Function} fn Passou o elemento criado e retornou um resultado booleano
 * /
afirmação de função (fn) {
 ...
 * @param {String} atrai a lista de atributos separados por tubulação
 manipulador * @param {Function} O método que será aplicado
 * /
função addHandle (attrs, manipulador) {
	var arr = attrs.split ("|"),
		i = comprimento arr;
...
/ **
 * Verifica a ordem dos documentos de dois irmãos
 * @param {Element} a
 * @param {Element} b
...
função siblingCheck (a, b) {
	...
	// Use o IE sourceIndex, se disponível nos dois nós
	if (diff) {
		retorno diff;
	}
 ...
 * tipo @param {String}
 * /
função createInputPseudo (type) {
	função de retorno (elem) {
		var name = elem.nodeName.toLowerCase ();
		nome do retorno === "input" && elem.type === type;
	};
}
 ...
 * tipo @param {String}
 * /
função createButtonPseudo (type) {
	função de retorno (elem) {
		var name = elem.nodeName.toLowerCase ();
 ...
 * @param {Boolean} desativado true para: disabled; false para: ativado
 * /
função createDisabledPseudo (desativado) {

	// Conhecido: desativado falsos positivos: fieldset [desativado]> legenda: n-é-tipo (n + 2): pode-desativar
	função de retorno (elem) {

		// Apenas certos elementos podem corresponder: ativado ou desativado
		// https://html.spec.whatwg.org/multipage/scripting.html#selector-enabled
		// https://html.spec.whatwg.org/multipage/scripting.html#selector-disabled
		if ("formulário" no elem) {

			// Verifique a deficiência herdada nos elementos não desativados relevantes:
			// * elementos associados ao formulário listados em um conjunto de campos desativado
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				// Os elementos de opção são adiados para um grupo de opções pai, se presente
				if ("label" no elem) {
				...
				// Suporte: IE 6 - 11
				// Use a propriedade de atalho isDisabled para verificar ancestrais de conjunto de campos desativados
				retornar elem.isDisabled === desativado ||

					// Onde não há isDisabled, verifique manualmente
					/ * jshint -W018 * /
					elem.isDisabled! ==! disabled &&
					inDisabledFieldset (elem) === desativado;
			}

			return elem.disabled === desativado;

		// Tente descobrir elementos que não podem ser desabilitados antes de confiar na propriedade desabilitada.
		// Algumas vítimas são pegas em nossa rede (etiqueta, legenda, menu, faixa), mas não devem
		// existe mesmo neles, quanto mais ter um valor booleano.
 ...
 * @returns {Elemento | Objeto | Booleano} O nó de entrada, se aceitável, caso contrário, um valor falso
 * /
função testContext (contexto) {
	retornar contexto && tipo de context.getElementsByTagName! == "undefined" && context;
}
...
isXML = Sizzle.isXML = function (elem) {
	...
	// Suporte: IE <= 8
	// Suponha HTML quando documentElement ainda não existir, como dentro de iframes de carregamento
 ...
 * /
setDocument = Sizzle.setDocument = função (nó) {
	var hasCompare, subWindow,
		doc = nó? node.ownerDocument || nó: preferidoDoc;

	// Retorna mais cedo se o documento for inválido ou já estiver selecionado
	// Suporte: IE 11+, Edge 17 - 18+
	// O IE / Edge às vezes gera um erro "Permissão negada" ao comparar estritamente
	...
	if (preferidoDoc! = documento &&
		...
		} senão se (subWindow.attachEvent) {
			subWindow.attachEvent ("onunload", unloadHandler);
		}
	}

	// Suporte: IE 8 - 11 ou superior, Edge 12 - 18 ou superior, Chrome <= 16 - 25 apenas, Firefox <= 3.6 - 31 apenas,
	// Somente Safari 4-5, Opera <= 11.6 - 12.x somente
	// IE / Edge e navegadores mais antigos não suportam a pseudo-classe: scope.
	// Suporte: somente Safari 6.0
	// O Safari 6.0 suporta: scope, mas é um alias de: root lá.
	support.scope = assert (função (el) {
	...
	// Os métodos getElementById quebrados não capturam nomes definidos programaticamente,
	// então use um teste getElementsByName da rotatória
	support.getById = assert (função (el) {
		docElem.appendChild (el) .id = expando;
		return! document.getElementsByName || ! document.getElementsByName (expando) .length;
	});

	// identifique o filtro e encontre
	if (support.getById) {
	...
	Expr.find ["TAG"] = support.getElementsByTagName?
		...
		função (tag, contexto) {
			var elem,
				...
				// Por feliz coincidência, um gEBTN (quebrado) também aparece nos nós do DocumentFragment
	...
	// Classe
	Expr.find ["CLASS"] = support.getElementsByClassName && função (className, contexto) {
	...
	// matchSelector (: active) reporta false quando verdadeiro (IE9 / Opera 11.5)
	rbuggyMatches = [];

	// qSa (: focus) relata falso quando verdadeiro (Chrome 21)
	// Permitimos isso por causa de um bug no IE8 / 9 que gera um erro
	// sempre que o `document.activeElement` for acessado em um iframe
	// Então, permitimos: o foco passe pelo QSA o tempo todo para evitar o erro do IE
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (função (el) {
			...
			// Select está definido como string vazia de propósito
			...
			docElem.appendChild (el) .innerHTML = "<a id='" + expando + "'> </a>" +
				"<selecione id = '" + expando + "- \ r \\' msallowcapture = ''>" +
				"<opção selecionada = ''> </option> </select>";

			// Suporte: IE8, Opera 11-12.16
			// Nada deve ser selecionado quando seqüências de caracteres vazias seguem ^ = ou $ = ou * =
			...
			// Curiosamente, o IE 10 e anteriores não parecem ter o problema.
			...
			// Webkit / Opera -: marcado deve retornar elementos de opção selecionados
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
			// O IE8 gera erro aqui e não verá testes posteriores
			if (! el.querySelectorAll (": selected") .length) {
			...
			// Falha no seletor `in-page # id sibling-combinator`
			if (! el.querySelectorAll ("a #" + expando + "+ *") .length) {
				rbuggyQSA.push (". #. + [+ ~]");
			}

			// Suporte: Firefox <= 3.6 - 5 apenas
			// O Firefox antigo não gera um identificador com escape grave.
		...
		assert (função (el) {
			...
			// Os atributos de tipo e nome são restritos durante a atribuição .innerHTML
			var input = document.createElement ("input");
			input.setAttribute ("type", "hidden");
			el.appendChild (input) .setAttribute ("nome", "D");

			// Suporte: IE8
			// Impor distinção entre maiúsculas e minúsculas do atributo name
			if (el.querySelectorAll ("[name = d]")) .length) {
				rbuggyQSA.push ("name" + espaço em branco + "* [* ^ $ |! ~]? =");
			}

			// FF 3.5 -: enabled /: desativado e elementos ocultos (os elementos ocultos ainda estão ativados)
			// O IE8 gera erro aqui e não verá testes posteriores
			...
			// Suporte: IE9-11 +
			// IE: o seletor desativado não pega os filhos dos conjuntos de campos desativados
			...
			// O Opera 10-11 não gera pseudos inválidos após a vírgula
			el.querySelectorAll ("* ,: x");
			rbuggyQSA.push (",. *:");
		});
	}

	if ((support.matchesSelector = rnative.test ((correspondências = docElem.matches ||
		...
		assert (função (el) {
			...
			support.disconnectedMatch = correspondências.call (el, "*");
	...
	// Classificação da ordem dos documentos
	...
	função (a, b) {
		...
		// Classifica a existência do método se apenas uma entrada tiver compareDocumentPosition
		var compare =! a.compareDocumentPosition -! b.compareDocumentPosition;
		if (compare) {
			retornar comparar;
		}

		// Calcular a posição se as duas entradas pertencerem ao mesmo documento
		// Suporte: IE 11+, Edge 17 - 18+
		// O IE / Edge às vezes gera um erro "Permissão negada" ao comparar estritamente
		// dois documentos; comparações superficiais funcionam.
		// eslint-disable-next-line eqeqeq
	...
	função (a, b) {
		...
		// Desce a árvore procurando discrepâncias
		while (ap [i] === bp [i]) {
			i ++;
		}
		...
		retornar eu?
			...
			// Verifica se os nós têm um ancestral comum
			siblingCheck (ap [i], bp [i]):

			// Caso contrário, os nós do nosso documento serão classificados primeiro
			// Suporte: IE 11+, Edge 17 - 18+
			// O IE / Edge às vezes gera um erro "Permissão negada" ao comparar estritamente
			// dois documentos; comparações superficiais funcionam.
			/ * eslint-disable eqeqeq * /
...
Sizzle.contains = function (contexto, elem) {
	...
	// Defina os vars do documento se necessário
	// Suporte: IE 11+, Edge 17 - 18+
	// O IE / Edge às vezes gera um erro "Permissão negada" ao comparar estritamente
	// dois documentos; comparações superficiais funcionam.
	// eslint-disable-next-line eqeqeq
...
Sizzle.attr = função (elem, nome) {
	...
	var fn = Expr.attrHandle [name.toLowerCase ()],
		...
		// Não se deixe enganar pelas propriedades de Object.prototype (jQuery # 13807)
...
/ **
 * Classificação e remoção de documentos duplicados
...
Sizzle.uniqueSort = function (results) {
	...
	// A menos que * saibamos * que podemos detectar duplicatas, assuma a presença deles
	...
	// Limpar entrada após a classificação para liberar objetos
...
/ **
 * Função utilitária para recuperar o valor do texto de uma matriz de nós DOM
 * @param {Array | Element} elem
 * /
getText = Sizzle.getText = função (elem) {
	nó var
		ret = "",
		i = 0
	...
	if (! nodeType) {
		...
		// Se no nodeType, espera-se que seja uma matriz
		while ((node ​​= elem [i ++])) {

			// Não atravessa nós de comentários
			ret + = getText (nó);
		}
	} else if (nodeType === 1 || nodeType === 9 || nodeType === 11) {

		// Use textContent para elementos
		// uso innerText removido para consistência de novas linhas (jQuery # 11153)
		if (typeof elem.textContent === "string") {
			retornar elem.textContent;
		} outro {

			// Atravessa seus filhos
	...
	// Não inclua comentário ou nós de instruções de processamento
...
Expr = Sizzle.selectors = {
	...
	preFilter: {
		"ATTR": função (correspondência) {
			match [1] = match [1]. substitua (runescape, funescape);

			// Mova o valor fornecido para corresponder a [3] entre aspas ou não.
			partida [3] = (partida [3] || partida [4] ||
				partida [5] || "") .place (runescape, funescape);
		...
		"CRIANÇA": função (correspondência) {
			...
			/ * correspondências de matchExpr ["CRIANÇA"]
				1 tipo (apenas | enésimo | ...)
				2 what (filho | do tipo)
				3 argumentos (pares | ímpares | \ d * | \ d * n ([+ -] \ d +)? | ...)
			...
			* /
			...
			if (combine [1] .slice (0, 3) === "enésimo") {
				...
				// lembre-se de que o elenco falso / verdadeiro é respectivamente 0/1
				partida [4] = + (partida [4]?
					correspondência [5] + (correspondência [6] || 1):
					2 * (corresponde a [3] === "par" || corresponde a [3] === "ímpar"));
				partida [5] = + ((partida [7] + partida [8]) || partida [3] === "ímpar");

				// outros tipos proíbem argumentos
			} senão se (corresponder a [3]) {
				Sizzle.error (partida [0]);
			}
		...
		"PSEUDO": function (match) {
			...
			if (matchExpr ["CHILD"] .test (partida [0])) {
				return null;
			}
			...
			// Retira caracteres em excesso de argumentos não citados
			} se if (sem aspas && rpseudo.test (sem aspas) &&
				...
				partida [0] = partida [0]. fatia (0, excesso);
				match [2] = fatia não cotada (0, excesso);
			}
	...
	filtro: {
		...
		"TAG": function (nodeNameSelector) {
			...
			retornar nodeNameSelector === "*"?
				...
				função (elem) {
					retornar elem.nodeName && elem.nodeName.toLowerCase () === nodeName;
				};
		}
		...
		"ATTR": função (nome, operador, cheque) {
			função de retorno (elem) {
				...
				/ * max-len eslint-desativar * /

				operador de retorno === "="? resultado === verificação:
					...
					operador === "| ="? resultado === verificação || result.slice (0, check.length + 1) === check + "-":
					falso;
		...
		"CRIANÇA": função (tipo, o que, _ argumento, primeiro, último) {
			var simple = type.slice (0, 3)! == "enésimo",
				forward = type.slice (-4)! == "last",
				ofType = what === "do tipo";

			retornar primeiro === 1 && ultimo === 0?
				...
				função (elem, _context, xml) {
					cache var, uniqueCache, outerCache, nó, nodeIndex, start,
					...
					if (pai) {
						...
						se (simples) {
							while (dir) {
								...
								// Inverta a direção para: only- * (se ainda não o fizemos)
						...
						if (forward && useCache) {
							...
							// Procura `elem` de um índice anteriormente armazenado em cache

							// ... de maneira amigável ao gzip
							nó = pai;
							...
							while ((node ​​= ++ nodeIndex && node && node [dir] ||
								...
								// Recurso à busca de elem desde o início
		...
		"PSEUDO": function (pseudo, argumento) {
			...
			// Priorize a distinção entre maiúsculas e minúsculas, caso pseudos personalizados sejam adicionados com letras maiúsculas
				...
				retornar Expr.setFilters.hasOwnProperty (pseudo.toLowerCase ())?
					markFunction (função (semente, correspondências) {
						var idx,
							...
							i = comprimento combinado;
						enquanto eu-- ) {
							idx = indexOf (semente, correspondida [i]);
							seed [idx] =! (corresponde a [idx] = corresponde a [i]);
						}
					}):
					função (elem) {
	...
	pseudos: {
		...
		"not": markFunction (função (seletor) {
			...
			retornar matcher [expando]?
				markFunction (função (semente, correspondências, _context, xml) {
					var elem,
						incomparável = correspondente (semente, nulo, xml, []),
						i = comprimento da semente;

					// Corresponde a elementos sem correspondência de `matcher`
					enquanto eu-- ) {
		...
		"has": markFunction (função (seletor) {
			função de retorno (elem) {
				retornar Sizzle (seletor, elem) .length> 0;
			};
		}),
		...
		// A correspondência de C com o valor da linguagem do elemento é realizada sem distinção entre maiúsculas e minúsculas.
			...
			função de retorno (elem) {
				...
				Faz {
					if ((elemLang = documentIsHTML?
						...
						retornar elemLang === lang || elemLang.indexOf (lang + "-") === 0;
					}
		...
		"marcado": function (elem) {

			// No CSS3,: selected deve retornar elementos selecionados e selecionados
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
		...
		"button": function (elem) {
			var name = elem.nodeName.toLowerCase ();
			nome do retorno === "input" && elem.type === "button" || nome === "botão";
		}

		"texto": função (elem) {
			var attr;
			retornar elem.nodeName.toLowerCase () === "input" &&
				elem.type === "texto" &&
		...
		"last": createPositionalPseudo (função (_matchIndexes, length) {
			retorno [comprimento - 1];
		}),

		"eq": createPositionalPseudo (função (_matchIndexes, length, argument) {
		...
		"ímpar": createPositionalPseudo (função (matchIndexes, length) {
			var i = 1;
			para (; i <comprimento; i + = 2) {
				matchIndexes.push (i);
			}
...
Expr.pseudos ["nésimo"] = Expr.pseudos ["eq"];

// Adicionar botão / tipo de entrada pseudos
para (i em {radio: true, caixa de seleção: true, arquivo: true, senha: true, imagem: true}) {
	Expr.pseudos [i] = createInputPseudo (i);
}
para (i em {submit: true, redefinir: true}) {
...
tokenize = Sizzle.tokenize = function (seletor, parseOnly) {
	var corresponde, corresponde, tokens, tipo,
		soFar, groups, preFilters,
		cache = tokenCache [seletor + ""];
	...
	while (soFar) {
		...
		if (! correspondido || (match = rcomma.exec (soFar))) {
			if (match) {
				...
				soFar = soFar.slice (corresponde a [0] .length) || tão longe;
			}
			groups.push ((tokens = []));
		}
		...
		if ((match = rcombinators.exec (soFar))) {
			...
			soFar = soFar.slice (comprimento combinado);
		}
	...
	// Retorna o tamanho do excesso inválido
	// se estamos apenas analisando
	// Caso contrário, gere um erro ou retorne tokens
	retornar parseOnly ?
		soFar.length:
		tão longe ?
...
função addCombinator (combinador, combinador, base) {
	...
	retornar combinator.first?
		...
		// Verifica o antepassado mais próximo / o elemento anterior
		função (elem, contexto, xml) {
			while ((elem = elem [dir])) {
				if (elem.nodeType === 1 || checkNonElements) {
					matcher de retorno (elem, contexto, xml);
				}
			}
			retorna falso;
		}:
		...
		função (elem, contexto, xml) {
			...
			// Não podemos definir dados arbitrários nos nós XML, para que eles não se beneficiem do cache do combinador
			if (xml) {
				while ((elem = elem [dir])) {
					if (elem.nodeType === 1 || checkNonElements) {
						if (correspondente (elem, contexto, xml)) {
							return true;
						}
					}
				}
				...
				while ((elem = elem [dir])) {
					if (elem.nodeType === 1 || checkNonElements) {
						...
						if (skip && skip === elem.nodeName.toLowerCase ()) {
							...
							// Atribua a newCache para que os resultados sejam propagados novamente para elementos anteriores
							return (newCache [2] = oldCache [2]);
						} outro {

							// Reutiliza newcache para que os resultados sejam propagados novamente para elementos anteriores
...
função multipleContexts (seletor, contextos, resultados) {
	var i = 0,
		len = contexts.length;
	para (; i <len; i ++) {
		Chiar (seletor, contextos [i], resultados);
	}
	retornar resultados;
}
...
função condensar (incomparável, mapa, filtro, contexto, xml) {
	...
	para (; i <len; i ++) {
		if ((elem = incomparável [i])) {
			if (! filter || filtro (elem, contexto, xml)) {
...
função setMatcher (preFilter, seletor, combinador, postFilter, postFinder, postSelector) {
	...
	return markFunction (função (semente, resultados, contexto, xml) {
		var temp, i, elem,
			...
			// Obter elementos iniciais da semente ou do contexto
			...
			// Pré-filtro para obter a entrada do correspondente, preservando um mapa para sincronização de resultados de sementes
			matcherIn = preFilter && (seletor seed ||!)?
				condensar (elems, preMap, preFilter, contexto, xml):
				elems,

			matcherOut = matcher?

				// Se tivermos resultados postFinder, semente filtrada ou pós-semente não semente, ou resultados pré-existentes,
				postFinder || (semente? preFilter: pré-existente || postFilter)?

					// ... é necessário processamento intermediário
					[]:
		...
		if (postFilter) {
			...
			// Cancele a correspondência dos elementos com falha, movendo-os de volta para matcherIn
			i = comprimento temp;
			enquanto eu-- ) {
				if ((elem = temp [i])) {
		...
		if (semente) {
			if (postFinder || preFilter) {
				if (postFinder) {
					...
					// Obtenha o matcherOut final condensando esse intermediário nos contextos postFinder
					temp = [];
					i = matcherOut.length;
					enquanto eu-- ) {
						if ((elem = matcherOut [i])) {

							// Restaurar matcherIn, já que elem ainda não é uma correspondência final
				...
				// Mova elementos correspondentes da semente para os resultados para mantê-los sincronizados
				i = matcherOut.length;
				enquanto eu-- ) {
					if ((elem = matcherOut [i]) &&
		...
		// Adicione elementos aos resultados, através do postFinder, se definido
		} outro {
...
função matcherFromTokens (tokens) {
	var checkContext, matcher, j,
		...
		// O correspondente de base garante que os elementos sejam alcançáveis ​​a partir do (s) contexto (s) de nível superior
		...
		matchers = [função (elem, contexto, xml) {
			var ret = (! leaderRelative && (xml || contexto! == outermostContext)) || (
				(checkContext = context) .nodeType?
					matchContext (elem, contexto, xml):
					matchAnyContext (elem, contexto, xml));

			// Evite pendurar no elemento (edição 299)
			checkContext = null;
			return ret;
		}];
		...
		if ((matcher = Expr.relative [tokens [i] .type])) {
			...
			if (matcher [expando]) {
				...
				retornar setMatcher (
					...
					// Se o token anterior for um combinador descendente, insira qualquer elemento implícito `*`
					fichas
						.slice (0, 1-1)
	...
	var bySet = setMatchers.length> 0,
		...
		superMatcher = função (semente, contexto, xml, resultados, mais externo) {
			...
			if (ultraperiférico) {
				...
				// dois documentos; comparações superficiais funcionam.
				// eslint-disable-next-line eqeqeq
				outermostContext = context == documento || contexto || mais externo;
			}

			// Adiciona elementos que passam elementMatchers diretamente aos resultados
			// Suporte: IE <9, Safari
			// Tolerar propriedades NodeList (IE: "length"; Safari: <number>) elementos correspondentes por ID
			for (; i! == len && (elem = elems [i])! = nulo; i ++) {
				...
				if (bySet) {
					...
					// Eles passaram por todos os possíveis jogadores
					if ((elem =! matcher && elem)) {
						matchedCount--;
					}

					// Aumenta a matriz para cada elemento, correspondido ou não
					if (semente) {
						unmatched.push (elem);
					}
				}
			...
			// `i` agora é a contagem de elementos visitados acima e o adiciona a` matchedCount`
			// torna o último não negativo.
			matchedCount + = i;

			// Aplicar filtros definidos a elementos sem correspondência
			// NOTA: Isso pode ser ignorado se não houver elementos incomparáveis ​​(por exemplo, `matchedCount`
			// é igual a `i`), a menos que não tenhamos visitado _any_ elementos no loop acima porque temos
			// nenhum elemento correspondente e nenhuma semente.
			// Incrementar uma string inicial "0" `i` permite que` i` permaneça uma string apenas nessa
			...
			if (porSet && i! == matchedCount) {
				...
				// O conjunto sem sementes corresponde a vários concorrentes bem-sucedidos estipulam a classificação
...
compile = Sizzle.compile = function (seletor, partida / * Somente para uso interno * /) {
	var i,
		setMatchers = [],
		elementMatchers = [],
 ...
 * função seletora criada com o Sizzle.compile
...
select = Sizzle.select = function (seletor, contexto, resultados, semente) {
	var i, tokens, token, tipo, localização,
		compilado = typeof selector === "function" && selector,
	...
	if (match.length === 1) {
		...
		// Reduza o contexto se o seletor principal composto for um ID
		fichas = correspondência [0] = correspondência [0]. fatia (0);
		if (tokens.length> 2 && (token = tokens [0]) .type === "ID" &&
			...
			// Matchers pré-compilados ainda verificarão a ancestralidade, portanto, aumentem
			} senão se (compilado) {
		...
		enquanto eu-- ) {
			...
			// Interrompa se atingirmos um combinador
			...
			if ((find = Expr.find [type])) {
				...
				if ((seed = find (
					...
					// Se a semente estiver vazia ou se não houver fichas, podemos retornar mais cedo
					tokens.splice (i, 1);
	...
	// Fornece `match` para evitar retokenization se modificamos o seletor acima
	(compilado || compilação (seletor, correspondência)) (
		semente,
		contexto,
		! documentIsHTML,
		resultados,
...
// Sempre assuma duplicatas se não forem passadas para a função de comparação
support.detectDuplicates = !! hasDuplicate;

// Inicializar no documento padrão
setDocument ();

// Suporte: Webkit <537.32 - Safari 6.0.3 / Chrome 25 (corrigido no Chrome 27)
// Nós desconectados seguem confusamente * uns aos outros *
support.sortDetached = assert (função (el) {

	// Deve retornar 1, mas retorna 4 (a seguir)
...
if (! assert (função (el) {
	...
	retornar el.firstChild.getAttribute ("href") === "#";
})) {
	addHandle ("tipo | href | altura | largura", função (elem, nome, isXML) {
		if (! isXML) {
			retornar elem.getAttribute (name, name.toLowerCase () === "type"? 1: 2);
		}
	});
}
...
if (! support.attributes ||! assert (function (el) {
	...
	retornar el.firstChild.getAttribute ("value") === "";
})) {
...
if (! assert (função (el) {
	retornar el.getAttribute ("desativado") == nulo;
})) {
	addHandle (booleanos, função (elem, nome, isXML) {
		var val;
		if (! isXML) {
			return elem [name] === verdadeiro? name.toLowerCase ():
				(val = elem.getAttributeNode (nome)) && val.especificado?
					val.value:
					nulo;
		}
...
var dir = função (elem, dir, até) {
	...
	while ((elem = elem [dir]) && elem.nodeType! == 9) {
		if (elem.nodeType === 1) {
			if (truncado && jQuery (elem) .is (até)) {
				quebrar;
			}
			pareado.push (elem);
		}
...
função winnow (elementos, qualificador, não) {
	...
	// Matriz de elementos (jQuery, argumentos, Matriz)
	if (typeof qualifier! == "string") {
		retornar jQuery.grep (elementos, função (elem) {
			return (indexOf.call (qualificador, elem)> -1)! == not;
		});
	}
...
jQuery.fn.extend ({
	find: function (seletor) {
		...
		if (typeof selector! == "string") {
			retorne this.pushStack (jQuery (seletor). filtro (função () {
				para (i = 0; i <len; i ++) {
		...
		para (i = 0; i <len; i ++) {
			jQuery.find (seletor, auto [i], ret);
		}
